...
HWND OSRWindow::CreateWidget(HWND hWndParent, const RECT& rect, LPCTSTR windowName, LPCTSTR className) {
		...
		HWND hWnd = ::CreateWindowEx(WS_EX_LEFT, className, windowName,
			WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_MINIMIZEBOX | WS_VISIBLE | WS_SIZEBOX | WS_OVERLAPPEDWINDOW,
...
bool OSRWindow::GetRootScreenRect(CefRefPtr<CefBrowser> browser,
	CefRect& rect) {
		...
		if (::GetClientRect(root_window, &window_rect)) {
...
void OSRWindow::OnPaint(CefRefPtr<CefBrowser> browser,
	...
	int width, int height) {
		...
		SetDIBitsToDevice( hdc, 0, 0, width, height, 0, 0, 0, height, buffer, &bmi, 0 );
...
// 					rc.bottom - rc.top), PET_VIEW);
				browser->Invalidate( PET_VIEW ); // bluker
			}
			return 0;
		}
		...
		case ID_OPENMAINWINDOW: {
			...
			CefBrowserHost::CreateBrowser(info, browser->GetClient().get(),
				L"http://www.baidu.com/", browser_settings, NULL);
			return 0;
		}
